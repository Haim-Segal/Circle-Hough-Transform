...
After creating the imperfect image, run CirclsDetector.m to detect the circles. the script will plot the detected circles and the rest of the unused pixels. in the function call you can insert the minimun number of dots needed to create a circle and also the minimum and maximun radius to set the radii range of circles you wish to find, or you can leave it empty to set the defult minimum of 4 dots and radii range between quarter and half of the image size. you can also insert only the first argument leaving the radii range to the default set.
